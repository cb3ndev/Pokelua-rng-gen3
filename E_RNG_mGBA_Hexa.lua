...
function on_frame()
    ...
    if prev_rng and (last_printed_frame ~= curr_frame) then
        ...
        if advances == 1 then
            print(string.format(
                "[RNG  NORMAL  ] Frame: %s | Avance: %s | RNG: %08X | Input: %s",
                pad(curr_frame, 6), pad(avance_actual, 6), curr_rng, input
            ))
        elseif advances > 1 then
            for i = 1, advances do
                local rng_val = get_nth_rng(prev_rng, i)
                print(string.format(
                    "[RNG MULTIPLE] Frame: %s | Avance: %s | RNG: %08X | (Avance #%d/%d) | Input: %s",
                    pad(curr_frame, 6), pad(calcular_avance_real(SEED_INICIAL, rng_val), 6), rng_val, i, advances, input
                ))
            end
        elseif advances == 0 then
            ...
            print(string.format(
                "[RNG ANÓMALO ] Frame: %s | Avance: %s | RNG: %08X -> %08X | Avances: ?? | Input: %s",
                pad(curr_frame, 6), pad(avance_actual, 6), prev_rng, curr_rng, input
            ))
        end
